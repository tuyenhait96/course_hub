...
// This will be passed as a prop from the server component
...
interface CourseDetailClientProps {
  allCoursesData: Record<string, Course>; // Add this to receive all courses data
}
              ...
              <ul className="list-disc list-inside space-y-2">
                {course.lessons.map((lesson) => (
                  <li key={lesson.id} className="text-gray-600">
                    <Link href={`/learner/lessons/${lesson.id}`} className="hover:text-indigo-600 hover:underline">
                      {lesson.title}
                    </Link>
                  </li>
                ))}
